...
Python数据预处理（归约、归一化、标准化）实现：极大型、极小型、区间型

```python
import numpy as np
# 数据预处理：极大极小型、区间型
# idx：第几个指标是这个类型的
def standard(data, max_idx, min_idx, range_idx, range_description):
    data = np.array(data)
...
min_idx = [3]
range_idx = [1]
range_description = [[5, 6]]
standard(data, max_idx, min_idx, range_idx, range_description)
```

> 预处理、归约、归一、标准化好像描述的都差不多吧，目前就这样理解了
...
> 上面这个式子在做**归一化**，可以先对每个方案都做一下，然后再**求距离**
...
data = standard(data, max_idx, min_idx, range_idx, range_description)
topsis(data, w)
```
